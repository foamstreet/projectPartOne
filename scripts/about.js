$(document).ready(function(){
  ...
  // Scene 2 - part 3 of 3 - slide 'I  design ...' out
  scenes[4] = new ScrollMagic.Scene({
    triggerElement: '#about-two',
    offset: 75
  })
  .setTween('#bio .about-right', .5, {width: 600, left: '45%'})
  .on('start', function(e) {
    if(e.scrollDirection === 'FORWARD'){
      $('#bio .about-right').hide().html('I design and<br>develop websites').fadeIn();
  ...
  // Scene 4  Resume
  scenes[7] = new ScrollMagic.Scene({
    triggerElement: '#about-four',
    offset: 175
  })
  .setTween('#about-four h2', .5, { scale: 1.5, opacity: 1 })
  .setPin('#about-four h2')
  .on('start', function(e) {
    if(e.scrollDirection === 'FORWARD'){
      $('body').css({'background-color':'#13C4A3'});
    }
    else {
      $('body').css({'background-color':'#AAA'});
    }
